...
Use pc_topic_search / pc_topic_get for requested Topic Memory, pc_experience_get / pc_skill_get for exact artifact inspection, and pc_review_list / pc_review_get for the candidate queue. Candidate inspection is available, but candidate generation and approval/rejection/revision are not exposed here. Never treat inspection as approval. Use pc_work_contract for explicitly delegated work, pc_handoff_acknowledge only after receiver checks, and pc_task_outcome at a real completion or interruption boundary.
