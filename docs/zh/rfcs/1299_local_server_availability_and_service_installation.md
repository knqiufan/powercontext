...
公开的生命周期命令组确定为 `powercontext service`。它不属于 `powercontext server`，只管理当前用户下、由
PowerContext 拥有的注册产物。默认不安装，且不需要 `root`、`SYSTEM` 或管理员账号。
...
`powercontext service status` 的范围比 `powercontext doctor` 更窄。它报告注册状态、manager 状态和 Server
liveness，但不替代 Server 的 readiness 诊断。
...
5. 为后续用户登录启用该注册。
6. 配置的 Server endpoint 尚未 live 时，默认立即启动服务。
...
distribution 保持一致。在 distribution 拥有事务化 upgrade hook 之前，文档必须包含这一 reconciliation 步骤。

## Failure handling and observability

原生启动失败继续出现在原生日志中。`powercontext service status` 和 `powercontext doctor` 会指出如何查看日志。
平台 adapter 返回结构化失败，
